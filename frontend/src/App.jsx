  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <header className={`shadow-md transition-colors duration-300 ${ darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        <div className="container mx-auto px-4 py-6 flex justify-between items-center">
          ...
          <div className="flex items-center">
            ...
            <button className={`ml-4 text-2xl focus:outline-none hover:bg-gray-600 ${darkMode ? 'text-white bg-gray-900 text-gray-100' : 'text-gray-600 bg-gray-100 text-gray-900'}`} onClick={toggleMenu}>
      ...
      <main className="container mx-auto px-4 py-8 flex flex-col md:flex-row">
          {/* Project Tab */}
          <div className={`md:w-1/4 mb-8 md:mb-0 md:mr-8 ${darkMode ? 'text-gray-300 bg-gray-800' : 'text-gray-800 bg-white'} flex flex-col rounded-lg shadow-md p-3 sticky top-8 transition-colors duration-300`}>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-row justify-between">
                      <div>
                        ...
                        <button className={`inline-block mx-2 p-3 rounded-xl transition-colors duration-300 ${ darkMode ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-yellow-400 hover:bg-yellow-500' } text-white`} onClick={() => toggleEditForm(index)}>
                          <FaPen />
                        </button>
                      </div>
                    ...
                    {editFormVisible[index] && (
                      <div className={`flex flex-col rounded-lg shadow-md p-3 mt-4 ${ darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
                        ...
                        <input className={`mx-2 h-8 p-3 mb-5 rounded-lg outline outline-gray-500 ${ darkMode ? 'text-white bg-gray-900 text-gray-100 outline-gray-500 hover:outline-gray-700' : 'text-gray-600 bg-gray-100 text-gray-900 outline-gray-200 hover:outline-gray-400'}`} type="text" placeholder="Task..." onChange={(e) => setNewTitle(e.target.value)} />

                        <input className={`mx-2 h-8 p-3 mb-5 rounded-lg outline outline-gray-500 ${ darkMode ? 'text-white bg-gray-900 text-gray-100 outline-gray-500 hover:outline-gray-700' : 'text-gray-600 bg-gray-100 text-gray-900 outline-gray-200 hover:outline-gray-400'}`} type="text" placeholder="Description..." onChange={(e) => setNewDescription(e.target.value)} />

                        <input className={`mx-2 h-8 p-3 mb-5 rounded-lg outline outline-gray-500 ${ darkMode ? 'text-white bg-gray-900 text-gray-100 outline-gray-500 hover:outline-gray-700' : 'text-gray-600 bg-gray-100 text-gray-900 outline-gray-200 hover:outline-gray-400'}`} type="text" placeholder="Due Date..." onChange={(e) => setNewCreatedat(e.target.value)} />

                        <label className={`mt-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                          Status:
                          <input id="check '{post.id}'" type="checkbox" checked={post.status} onChange={() => toggleStatus(post.id, post.status)}className="text-xl m-3" />
                        </label>
